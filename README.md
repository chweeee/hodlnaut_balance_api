...
> 1) Maybe try exploring implementing the same thing use websocket server,
> 2) Maybe design more complicated test cases to check for validity of lastPrice. Current tests only checks that HTTP requests went through nicely.

__2. Which took the most time? What did you find most difficult?__
> Writing tests for app routes that calls async functions using Chai + Mocha as I had very little prior experience using them.
...
> Instead of using a HTTP server, we could use a WebSocket server instead, maybe something like: https://github.com/websockets/ws#simple-server. Using a websocket server is more 
  efficient that sending HTTP requests 10 times, as HTTP requests are "heavier" (request/response headers, etc...), which information we do not need. Using a persistent connection
  is way better for a streaming use-case.
...
> It was honestly fun and a good way to get my hands dirty writing tests with Chai and Mocha
